...
4. **escape IDL-keyword struct names** — `Bool`, `Byte`, `Char`, `String` in `std_msgs` collide with IDL keywords case-insensitively. We append a trailing underscore (`String_`) — **not** a leading one, because XTypes 1.3's `is_type_name_consistent` FSM rejects identifiers starting with `_` after `::`, which would break Fast-DDS's TypeDescriptor validation inside ddsrecorder.
